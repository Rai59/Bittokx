...
function owl(fill) {
  ...
  <g transform="translate(256 258)">
    <path d="M-160 -118 L0 -70 L160 -118" fill="none" stroke="${fill}" stroke-width="38" stroke-linecap="round" stroke-linejoin="round"/>
    <path fill="${fill}" fill-rule="evenodd" d="M-94 24 m-80 0 a80 80 0 1 0 160 0 a80 80 0 1 0 -160 0 M-78 29 m-13 0 a13 13 0 1 0 26 0 a13 13 0 1 0 -26 0"/>
    <circle cx="94" cy="24" r="58" fill="none" stroke="${fill}" stroke-width="44"/>
    <circle cx="74" cy="28" r="13" fill="${fill}"/>
    <path d="M-22 64 L22 64 L0 122 Z" fill="${fill}"/>
  </g>`;
}
...
fs.writeFileSync(
  path.join(out, "bittokx-mark.svg"),
  svg(512, 512, owl("#111111"), "Bittokx mark")
);
fs.writeFileSync(
  path.join(out, "bittokx-mark-white.svg"),
  svg(512, 512, owl("#FFFFFF"), "Bittokx mark, white")
);
fs.writeFileSync(
  path.join(out, "bittokx-app-icon.svg"),
  svg(
    512,
    512,
    `  <rect width="512" height="512" rx="112" fill="#111111"/>
  <g transform="translate(46 40) scale(0.82)">
${owl("#FFFFFF")}
  </g>`,
    "Bittokx app icon"
  )
...
const lockup = (fill) => `
  <g transform="translate(8 2) scale(0.48)">
${owl(fill)}
  </g>
  <path transform="translate(268 178)" fill="${fill}" d="${WORDMARK}"/>
`;

fs.writeFileSync(
  path.join(out, "bittokx-logo.svg"),
  svg(900, 280, lockup("#111111"), "Bittokx logo")
);
fs.writeFileSync(
  path.join(out, "bittokx-logo-white.svg"),
  svg(900, 280, lockup("#FFFFFF"), "Bittokx logo, white")
);
